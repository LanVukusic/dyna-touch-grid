 ...
 */
export const DraggableGridItem = ({ instance, renderers, cellSize, gap = 0, }) => {
    const { id, position } = instance;
    // Draggable hook for moving the entire item.
    const { attributes: moveAttributes, listeners: moveListeners, setNodeRef: setMoveNodeRef, isDragging: isMoving, transform: moveTransform, } = useDraggable({
        id: `move-${id}`,
        data: { type: 'move', instance },
    });
    // Draggable hook specifically for the resize handle.
    const { attributes: resizeAttributes, listeners: resizeListeners, setNodeRef: setResizeNodeRef, isDragging: isResizing, transform: resizeTransform, } = useDraggable({
        id: `resize-${id}`,
    ...
    _jsx("div", { ref: setMoveNodeRef, style: style, ...moveListeners, ...moveAttributes, children: _jsxs("div", { style: {
                position: 'relative',
                width: '100%',
                height: '100%',
            }, children: [_jsx(MoveOverlay, { isMoving: isMoving }), _jsx("div", { ref: setResizeNodeRef, ...resizeListeners, ...resizeAttributes, children: _jsx(ResizeHandle, { isResizing: isResizing }) })] }) }));
};
